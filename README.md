...
- 🌐 **传输协议选择**: 支持 TCP, WebSocket, WSS 传输协议，可轻松配合 Cloudflare 等 CDN 进行内网穿透
...
### v1.1.0 (2026-06-13)
- ✨ 新增传输协议选择支持 (TCP / WebSocket / WSS)
- 🌐 支持配置 WebSocket 和 TLS，轻松配合 Cloudflare CDN 进行穿透转发
- 🔒 新增 CDN 兼容端口的验证与提示
- ⚡ 支持配置自定义 DNS 解析服务器（dnsServer），有效规避 Clash 等 Fake-IP 模式导致的解析环路与连不上问题
- 📊 状态监控界面支持显示当前传输协议与自定义 DNS 状态
